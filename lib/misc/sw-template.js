...
function WebpackServiceWorker(params) {
  ...
  self.addEventListener('fetch', function (event) {
    ...
    if (event.request.method !== 'GET' || url.origin !== location.origin || allAssets.indexOf(url.pathname) === -1) {
      ...
      if (/\/[0-9a-f]{8}-([0-9a-f]{4}-){3}[0-9a-f]{12}/.test(url.pathname)) {
    ...
    var resource = caches.match(event.request, {
      ...
      return fetch(event.request.clone()).then(function (response) {
        ...
        caches.open(CACHE_NAME).then(function (cache) {
          ...
          // this happens when one manually deletes the cache :(
        });

        return response;
      })['catch'](function (err) {
        console.log('[SW]: Error fetch - ' + err);
      });
    })['catch'](function (err) {
      console.log('[SW]: Error caches.match - ' + err);
    });
